...
@section('pageTitle', 'Add Student - Student Profile Management')

@section('content')
  <div class="card card-primary">
    <div class="card-header">
      <h3 class="card-title">ប្រវត្តិរូបសិស្ស / Student Registration</h3>
    </div>
              ...
              <div class="col-7">
                <select name="birth_province_id" class="tom-select province-select" data-target="birth_district_id">
                  ...
                  @foreach ($provinces as $province)
                    <option value="{{ $province->id }}" {{ old('birth_province_id') == $province->id ? 'selected' : '' }}>
            ...
            <div class="form-group row align-items-center mb-2">
              ...
              <div class="col-7">
                <select name="birth_village_id" id="birth_village_id" class="tom-select"
                  data-selected="{{ old('birth_village_id') }}">
                ...
                <select name="current_province_id" class="tom-select province-select" data-target="current_district_id">
                  ...
                  @foreach ($provinces as $province)
                    <option value="{{ $province->id }}"
                      {{ old('current_province_id') == $province->id ? 'selected' : '' }}>{{ $province->name_kh }}
                    </option>
                  @endforeach
                </select>
              </div>
          ...
          <div class="col-md-6 pl-md-4">
            ...
            <div class="form-group row align-items-center mb-2">
              ...
              <div class="col-7">
                <input type="text" name="guardian_relationship" class="form-control form-control-sm"
                  value="{{ old('guardian_relationship') }}" placeholder="e.g. Father, Mother">
              </div>
            </div>
            ...
            <div class="row mb-3">
              <div class="col-7">
                ...
                <textarea name="note" class="form-control form-control-sm" rows="5">{{ old('note') }}</textarea>
              </div>
...
@push('scripts')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Pre-populate cascading location selects on initial load (after validation errors)
      document.querySelectorAll('.province-select').forEach(function(sel) {
        var ts = tsInstance(sel);
