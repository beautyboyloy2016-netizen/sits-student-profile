...
@section('pageTitle', 'Edit Class - Student Profile Management')
    ...
    <div class="col-md-12">
      <div class="card card-primary">
        <div class="card-header">
          <h3 class="card-title">Edit Class: {{ $class->class_code }}</h3>
        </div>
            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <input type="text" name="class_code" class="form-control" value="{{ old('class_code', $class->class_code) }}" required>
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <select name="level_id" class="form-control tom-select">
                    ...
                    @foreach($levels as $level)
                      <option value="{{ $level->id }}" {{ old('level_id', $class->level_id) == $level->id ? 'selected' : '' }}>{{ $level->name }} ({{ $level->course->name ?? '' }})</option>
                    @endforeach
                  </select>
                </div>
              </div>
                ...
                <div class="form-group">
                  ...
                  <select name="academic_year_id" class="form-control tom-select">
                    ...
                    @foreach($academicYears as $year)
                      <option value="{{ $year->id }}" {{ old('academic_year_id', $class->academic_year_id) == $year->id ? 'selected' : '' }}>{{ $year->name }} {{ $year->is_current ? '(Current)' : '' }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
                ...
                <div class="form-group">
                  ...
                  <select name="teacher_id" class="form-control tom-select">
                    ...
                    @foreach($teachers as $teacher)
                      <option value="{{ $teacher->id }}" {{ old('teacher_id', $class->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name_kh }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
                ...
                <div class="form-group">
                  ...
                  <select name="room_id" class="form-control tom-select">
                    ...
                    @foreach($rooms as $room)
                      <option value="{{ $room->id }}" {{ old('room_id', $class->room_id) == $room->id ? 'selected' : '' }}>{{ $room->room_no }} ({{ $room->building->name ?? '' }})</option>
                    @endforeach
                  </select>
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <input type="text" name="start_date" class="form-control flatpickr" value="{{ old('start_date', $class->start_date) }}">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label>End Date</label>
                  <input type="text" name="end_date" class="form-control flatpickr" value="{{ old('end_date', $class->end_date) }}">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
