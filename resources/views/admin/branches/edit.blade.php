...
@section('pageTitle', __('app.edit_branch') . ' - ' . __('app.app_name'))
@section('pageHeading', __('app.edit_branch') . ': ' . $branch->name_en)

@section('breadcrumb')
  <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('app.dashboard') }}</a></li>
  <li class="breadcrumb-item"><a href="{{ route('branches.index') }}">{{ __('app.branches') }}</a></li>
  <li class="breadcrumb-item active">{{ __('app.edit_branch') }}: {{ $branch->name_en }}</li>
@endsection
    ...
    <div class="col-md-9 col-lg-8">
      <div class="card card-warning card-outline">
        <div class="card-header">
          <h3 class="card-title"><i class="fas fa-edit mr-2"></i>{{ __('app.edit_branch') }}:
            <strong>{{ $branch->name_kh }}</strong></h3>
        </div>
        <div class="card-body">
          <form action="{{ route('branches.update', $branch->id) }}" method="POST" autocomplete="off">
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <input type="text" name="name_kh" class="form-control"
                    value="{{ old('name_kh', $branch->name_kh) }}" required>
                  @error('name_kh')
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <input type="number" name="sort_order" class="form-control"
                    value="{{ old('sort_order', $branch->sort_order) }}" min="0">
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
            ...
            <div class="form-group mt-3">
              <button type="submit" class="btn btn-warning">
                <i class="fas fa-save mr-1"></i> {{ __('app.update') }}
              </button>
